...
KB_FILE = BASE_DIR / "spotbot_knowledge" / "general" / "knowledge_base.json"
...
async def home(request: Request):
    ...
    if spot_key:
        ...
        # CACHE MANAGEMENT
        # If cache gets too big, clears the oldest entry
...
async def get_vibe_api(spot: str, weight: str = "75", level: str = "intermediate", discipline: str = "auto"):
    try:
        ...
        # Check if this report in memory
        report_data = REPORT_CACHE.get(cache_key) or await get_shred_report(spot, weight, level, discipline)
        
        final_disc = report_data['live']['recommended_gear']['type']

        vibe = await get_ai_recommendation(
            report=report_data, 
            user_weight=int(weight) if weight.isdigit() else 75, 
            spot_key=spot, 
            user_level=level, 
